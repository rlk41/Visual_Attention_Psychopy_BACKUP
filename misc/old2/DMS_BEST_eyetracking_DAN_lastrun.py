...
expName = 'DMS_BEST_eyetracking_DAN'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/kleinrl/Visual_Attention/DMS_BEST_eyetracking_DAN_lastrun.py',
...
ioServer = io.launchHubServer(window=win, experiment_code='DMS_BEST_eyetracking_DAN', session_code=ioSession, datastore_name=filename, **ioConfig)
...
for thisTrial in trials:
    ...
    # keep track of which components have finished
    trialComponents = [etRecord, sample, Delay, Response_1, Response_2, Response_3, polygon, etrecord_pos]
    ...
    while continueRoutine:
        ...
        if etrecord_pos.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > etrecord_pos.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
...
for thisTrial_2 in trials_2:
    ...
    while continueRoutine:
        ...
        if etrecord_pos_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > etrecord_pos_2.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
...
for thisTrial_3 in trials_3:
    ...
    while continueRoutine:
        ...
        if etrecord_pos_3.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > etrecord_pos_3.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
...
for thisTrial_4 in trials_4:
    ...
    while continueRoutine:
        ...
        if etrecord_pos_4.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > etrecord_pos_4.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
...
thisExp.saveAsPickle(filepath+"/thisExp.pkl")
