...
#trialNum = 30 # 9 trials * 4sec/trial = 36 sec + 12 sec baseline
#trialNum = 12 # 9 trials * 4sec/trial = 36 sec + 12 sec baseline
#trialNum = 9 # 9 trials * 4sec/trial = 36 sec + 12 sec baseline

#trialNum = 30 # 9 trials * 4sec/trial = 36 sec + 12 sec baseline
#trialNum = 12 # 9 trials * 4sec/trial = 36 sec + 12 sec baseline
#trialNum = 18 # 18 trials * 2sec/trial = 36 sec + 12 sec baseline
...
#paradigm_name="stimBlast"
paradigm_name="stimBlast2"

#paradigm_name = 'testing'
#paradigm_name = 'bastos'

if paradigm_name == "testing":
    print("testing")
    trialNum=2
    baseline_duration=2 
    blockNum=2
...
# bastos timings 
    ...
    delay_resp_period = 2
    rest_dur=1
    print("using bastos timings ")


elif paradigm_name == "stimBlast":
    trialNum = 30 
    cue_onset=0
    cue_dur=.2
    delayTime = random.uniform(0.3, .6)   
    delay_resp_period = .8
    rest_dur=0.2
    
    total_trial_time = cue_dur + \
    delay_resp_period + \
    rest_dur
    print("using stimBlast timings ")


elif paradigm_name == "stimBlast2":
    trialNum = 40
    cue_onset=0
    cue_dur=.1
    delayTime = random.uniform(0.3, .5)   
    delay_resp_period = .7
    rest_dur=0.1
    ...
    delay_resp_period + \
    rest_dur
    print("using stimBlast2 timings ")

else: 
    print("NO PARADIGM NAME" )
    exit

        # | cue_dur (0.2s) | delay_resp_period (1s) | rest (0.2s) |
        # | cue_dur (0.2s) | delay + resp           | 

#        delayTime = random.uniform(0.5, 1.2)
#        cue_dur=1
#        cue_dur=0.5
#        rest_dur=0.5
#        delay_response_total_time = 2
...
#        
#        
#        circle_onset=cue_onset
#        circle_dur=(cue_onset + cue_dur)+delayTime-.25 
#        
#        delay_onset=(cue_onset + cue_dur)
#        delay_dur=delayTime
#        
#        resp_onset=delay_onset + delayTime  
#        resp_dur=delay_resp_period - delayTime
#        
#        rest_onset=resp_onset + resp_dur
#        
#        cross_onset=rest_onset 
#        cross_dur=rest_dur

    
blockTypes=["PRED","UNPRED",
"PRED","UNPRED",
"PRED","UNPRED",
"PRED","UNPRED",
"PRED","UNPRED",
...
for i_block in range(blockNum):
    ...
    if blockType == "PRED": 
        ...
        #stimImage = 'images/picture_2.png' #pictures[stimImage_i]
...
#        rest_dur=0.2
        ...
        total_dur = cross_onset+cross_dur
        
        stim_onset = total_running + cue_onset
        stim_dur = cue_dur
        
        total_running += total_dur 

        
        print("total time: {}".format(total_dur))
        ...
        if responseImage1 == stimImage:
            targImage       = responseImage1 
            targImage_i     = responseImage1_i
            targImage_pos   = responseImage1_pos
            targImage_X     = responseImage1_X
            targImage_Y     = responseImage1_Y
        elif responseImage2 == stimImage:
            targImage       = responseImage2 
            targImage_i     = responseImage2_i
            targImage_pos   = responseImage2_pos
            targImage_X     = responseImage2_X
            targImage_Y     = responseImage2_Y           
        elif responseImage3 == stimImage:
            targImage       = responseImage3 
            targImage_i     = responseImage3_i
            targImage_pos   = responseImage3_pos
            targImage_X     = responseImage3_X
            targImage_Y     = responseImage3_Y      
        else:
            targImage       = "NA" 
            targImage_i     = "NA"
            targImage_pos   = "NA"
        ...
        app = { "paradigm_name":paradigm_name,
        "trialNum":trialNum, 
        "cue_onset":cue_onset, 
        "cue_dur":cue_dur, 
        "delayTime":delayTime,
        "delay_resp_period":delay_resp_period,
        "rest_dur":rest_dur,
        ...
        "targImage": targImage, 
        "targImage_i": targImage_i, 
        "targImage_pos":targImage_pos, 
        "targImage_X": targImage_pos, 
        "targImage_Y": targImage_pos, 
...
expName = 'DMS_etROI_12-36_stimBlast'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/richard/Visual_Attention/DMS_etROI_12-36_stimBlast_lastrun.py',
...
ioServer = io.launchHubServer(window=win, experiment_code='DMS_etROI_12-36_stimBlast', session_code=ioSession, datastore_name=filename, **ioConfig)
...
roi = visual.ROI(win, name='roi', device=eyetracker,
    debug=False,
    shape='triangle',
    pos=[0,0], size=responseImage_size, anchor='center', ori=0.0)
...
for thisBlock in blocks:
    ...
    for thisTrial in trials:
        ...
        roi.setPos([thisTrial['targImage_pos']])
        # clear any previous roi data
        roi.reset()
        # keep track of which components have finished
        trialComponents = [sample, Delay, Response_1, Response_2, Response_3, polygon, roi]
        ...
        while continueRoutine:
            ...
            # *sample* updates
            if sample.status == NOT_STARTED and tThisFlip >= thisTrial['cue_onset']-frameTolerance:
                # keep track of start time/frame for later
            ...
            if sample.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > sample.tStartRefresh + thisTrial['cue_dur']-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # *polygon* updates
            if polygon.status == NOT_STARTED and tThisFlip >= thisTrial['cross_onset']-frameTolerance:
                # keep track of start time/frame for later
            ...
            if polygon.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > polygon.tStartRefresh + thisTrial['cross_dur']-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if roi.status == NOT_STARTED and tThisFlip >= thisTrial["resp_onset"]-frameTolerance:
                # keep track of start time/frame for later
            ...
            if roi.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > roi.tStartRefresh + thisTrial["resp_dur"]-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    outfile = filename+"_thisExp.pkl"
    thisExp.saveAsPickle(outfile)
    print("saving thisExp as pkl here: {}".format(outfile))
